...
def risk_free_rate_daily_flow() -> None:
    ...
    db.execute_sql_template_file(
        "pipelines/ibkr/sql/risk_free_rate_merge.sql",
        params={"stage_table": stage_table},
    )
...
def risk_free_rate_backfill_flow(start_date: dt.date, end_date: dt.date) -> None:
    # 1. Get risk free rate data
