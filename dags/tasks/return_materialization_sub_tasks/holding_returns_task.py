...
def holding_return_materializations_daily() -> None:
    ...
    db.execute_sql_file('pipelines/ibkr/sql/returns_create.sql')

    # 2. Materialize table
    db.execute_sql_template_file(
        file_name='pipelines/ibkr/sql/returns_materialize.sql',
        params={'start_date': last_market_date, 'end_date': last_market_date}
    )

@task(task_id="holding_return_materializations")
def holding_return_materializations_backfill(start_date: dt.date, end_date: dt.date) -> None:
    # 1. Create core table if not exists
    db = aws.RDS(
        db_endpoint=os.getenv("DB_ENDPOINT"),
        db_name=os.getenv("DB_NAME"),
