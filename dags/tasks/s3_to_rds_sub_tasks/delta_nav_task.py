...
def delta_nav_transform_and_load_backfill(from_date: dt.date, to_date: dt.date):
    # 1. Process raw delta_nav data from all S3 sources
    fs = fsspec.filesystem("s3", **storage_options)

    backfill_files = fs.glob(
        f"s3://ibkr-flex-query-files/backfill-files/{from_date}_{to_date}/*/*-delta_nav.csv"
    )
    history_files = fs.glob(
        "s3://ibkr-flex-query-files/history-files/*/*delta_nav*.csv"
    )
    daily_files = fs.glob("s3://ibkr-flex-query-files/daily-files/*/*/*delta_nav.csv")
